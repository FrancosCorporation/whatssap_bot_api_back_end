...
const apiUrl = process.env.API_URL; // URL da API de IA 
const nomeDoModelo = 'Cogito'; // Aqui você pode configurar o modelo de IA que usar
/**
 * Faz uma requisição para a IA usando dados da web + mensagem do usuário
 * @param {string} messageUser - Pergunta/mensagem original do usuário
 * @returns {Promise<string>} - Resposta da IA ou mensagem de erro
 */
async function perguntarIA(messageUser) {
  try {
    ...
    return respostaIA.data.response || '⚠️ Resposta vazia da IA.';
  } catch (erro) {
...
module.exports = { perguntarIA,buscarResumoWeb  };
